...
namespace NMib::NBuildSystem::NRepository
{
	...
	bool fg_IsSubmodule(CStr const &_DataDir)
	{
		...
		return _DataDir.f_Find("/.git/modules/") >= 0;
	}
	...
	bool fg_IsWorktree(CStr const &_DataDir)
	{
		...
		// Submodules have data dir pointing to <main>/.git/modules/<name>
		return _DataDir.f_Find("/.git/worktrees/") >= 0;
	}
	...
	TCUnsafeFuture<TCVector<CStr>> fg_ListWorktreePaths(CGitLaunches &_Launches, CStr const &_RepoDir)
	{
		...
		auto Result = co_await _Launches.f_Launch(_RepoDir, {"worktree", "list", "--porcelain"});
		...
		for (auto &Line : Result.f_GetStdOut().f_SplitLine<true>())
		{
			if (Line.f_StartsWith("worktree "))
				Paths.f_Insert(Line.f_Extract(9));
		}
	...
	TCUnsafeFuture<void> fg_TransferGitDirMainToWorktree(CGitLaunches &_Launches, CStr const &_MainSubRepoDir, CStr const &_TargetWorktreeSubRepoDir)
	{
		...
		co_await _Launches.f_Launch(_TargetWorktreeSubRepoDir, {"worktree", "repair"});
	...
	TCUnsafeFuture<void> fg_TransferGitDirWorktreeToMain(CGitLaunches &_Launches, CStr const &_WorktreeSubRepoDir, CStr const &_MainSubRepoDir, CStr const &_WorktreeName)
	{
		...
		auto RevParseResult = co_await _Launches.f_Launch(_MainSubRepoDir, {"rev-parse", "HEAD"});
		...
		co_await _Launches.f_Launch(_MainSubRepoDir, {"worktree", "repair"});
	...
	TCFuture<bool> fg_RepoIsChanged(CGitLaunches _GitLaunches, CRepository _Repo, CRepositoryDynamicInfo _DynamicInfo, EFilterRepoFlag _Flags, bool _bIncludePull, CStr _MainRepoBranch, CStr _MainRepoDefaultBranch)
	{
		...
		else if (BranchLine.f_StartsWith("## "))
		{
			if (BranchLine.f_Find(gc_ConstString_Symbol_Ellipsis.m_String) < 0)
			{
				...
				{
					auto MergeBaseResult = co_await _GitLaunches.f_Launch(_Repo, {"merge-base", "--is-ancestor", ExpectedBranch, "origin/{}"_f << _Repo.m_OriginProperties.m_DefaultBranch});
	...
	{
		if (_MainRepoBranch.f_IsEmpty() || _MainRepoDefaultBranch.f_IsEmpty() || _MainRepoBranch == _MainRepoDefaultBranch)
	...
	CMainRepoBranchInfo fg_GetMainRepoBranchInfo(CStr const &_BaseDir, TCVector<TCMap<CStr, CReposLocation>> const &_ReposOrdered)
	{
		...
		if (CFile::fs_FileExists(_BaseDir + "/.git", EFileAttrib_Directory | EFileAttrib_File))
		{
			...
			CStr HeadRef = CFile::fs_ReadStringFromFile(DataDir + "/HEAD", true).f_TrimRight("\n");
			if (HeadRef.f_StartsWith("ref: "))
				(CStr::CParse("ref: refs/heads/{}") >> Info.m_Branch).f_Parse(HeadRef);
		}
