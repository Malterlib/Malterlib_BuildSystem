...
namespace NMib::NBuildSystem
{
	...
	void CBuildSystem::fp_UpdateDependenciesNames(CEntity *_pTargetOuterEntity) const
	{
		...
		PropertyKey.m_Name = "DependenciesNames";
	...
	{
		CStr Name = _Entity.f_GetKeyName();
		...
		if (!pChildEntity)
		{
			...
			if (!f_EvalCondition(*pNewEntity, pNewEntity->f_Data().m_Condition, Target.f_Data().m_Debug.f_Find("TraceCondition") >= 0))
			...
			while (bAdded)
			{
				fp_UpdateDependenciesNames(pParent);

				bAdded = false;
				CProperty const *pFromProp = nullptr;
				auto ExtraGroups = f_EvaluateEntityPropertyUncachedStringArray(*pNewEntity, EPropertyType_Target, "ExtraGroups", pFromProp, nullptr, TCVector<CStr>());
				...
				for (auto &Group : ExtraGroups)
				{
					...
					if (!pGroup)
						fsp_ThrowError(pFromProp ? pFromProp->m_Position : EntityData.m_Position, CStr::CFormat("Extra group '{}' not found") << Group);
					...
					else
					{
						...
						if (pParentGroup->m_ChildEntitiesMap.f_FindEqual(Key))
							fsp_ThrowError(pFromProp ? pFromProp->m_Position : EntityData.m_Position, CStr::CFormat("Duplicate extra group '{}'") << Group);
						CEntity const *pSource;

						pSource = &GroupEntity;

						pParentGroup->m_ChildEntitiesMap(Key, *pSource, pParentGroup, EEntityCopyFlag_CopyChildren);
					}
				}
			}
		}
