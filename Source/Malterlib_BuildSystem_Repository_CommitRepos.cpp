...
namespace NMib::NBuildSystem
{
	...
	namespace
	{
			...
			)
		{
			...
			if (!_RepoMergeBase.f_IsEmpty() && !_CommitAnalyses.f_IsEmpty())
			{
				...
				TCSet<CStr> HeadMissingPathSet;
				{
					...
					for (auto &Path : _AllConfigRelPaths)
					{
						...
						if (!co_await fg_PathExistsAtCommit(_Launches, _Repo, "HEAD", Path))
							HeadMissingPathSet[Path];
					}
				...
				// Commit the reset (diff: HEAD state → merge-base state). Skip
				// paths that exist in neither HEAD nor the merge-base — the
				// temp-reset commit has nothing to record for them, and after
				// the delete step above the path is also gone from the working
				// tree and from the post-`git reset` index, so
				// `git add --all -- <path>` would error with "did not match any
				// files". This covers both untracked-and-newly-added mirrors
				// (e.g. files freshly generated by update-repos) and the case
				// where the user pre-staged such a file before invoking
				// repo-commit. The "Update repositories" commit below restores
				// them from DesiredConfigContents and stages them as fresh
				// additions.
				{
					TCVector<CStr> AddParams = {"add", "--all", "--"};
					for (auto &Path : _AllConfigRelPaths)
					{
						if (HeadMissingPathSet.f_FindEqual(Path))
							continue;
						AddParams.f_Insert(Path);
					}
					if (AddParams.f_GetLen() > 3)
						co_await _Launches.f_Launch(_Repo, AddParams, {}, CProcessLaunchActor::ESimpleLaunchFlag_GenerateExceptionOnNonZeroExitCode);
				}
			...
			{
				// `git checkout HEAD -- <paths>` rather than `git reset HEAD --
				// <paths>`: both sync the index entry to HEAD's blob, but reset
				// leaves the worktree file's mtime/inode untouched, so external
				// tools that watch on-disk state (e.g. Sublime Merge) keep
				// flagging the freshly-committed managed config files as
				// "changed since checkout" until the next git refresh. Doing a
				// real checkout rewrites both index and worktree from HEAD, so
				// stat info matches and clients agree the file is clean. For
				// paths that weren't touched in this run the no-op checkout is
				// cheap.
				TCVector<CStr> CheckoutParams = {"checkout", "--quiet", "HEAD", "--"};
				for (auto &Path : _AllConfigRelPaths)
					CheckoutParams.f_Insert(Path);
				co_await _Launches.f_Launch(_Repo, CheckoutParams, {}, CProcessLaunchActor::ESimpleLaunchFlag_None);
			}
		...
		)
	{
		...
		// AdditionalConfigFiles are mirror copies (typically in the root repo for
		// GitHub Actions cache keying). They are tracked in a parallel
		// AdditionalFilesByRepo map so they get staged and committed in lockstep
		// with the owning .MRepo, but they are deliberately kept out of the
		// canonical ConfigFilesByRepo / ReposByConfigFile views — those drive
		// start-commit resolution and child-commit collection, where parsing a
		// mirror would duplicate the canonical's repo-hash entries and produce
		// duplicated log queries / commit-message sections.
		TCMap<CStr, TCSet<CStr>> ConfigFilesByRepo;
		TCMap<CStr, TCSet<CStr>> AdditionalFilesByRepo;
		TCSet<CStr> OrphanedConfigFiles;
		auto fAssignConfigFile = [&](CStr const &_ConfigFile, TCMap<CStr, TCSet<CStr>> &o_FilesByRepo)
			{
				CStr ConfigDirectory = CFile::fs_GetPath(_ConfigFile);
				CStr OwnerLocation;
				auto *pOwner = CBuildSystem::fs_FindContainingPath(*pRepositoryByLocation, ConfigDirectory, OwnerLocation);
				if (pOwner)
					o_FilesByRepo[(*pOwner)->m_Location][_ConfigFile];
				else
					OrphanedConfigFiles[_ConfigFile];
			}
		;
		for (auto &pRepo : *pRepositoryByLocation)
		{
			...
			if (!Repo.m_ConfigFile.f_IsEmpty())
				fAssignConfigFile(Repo.m_ConfigFile, ConfigFilesByRepo);

			for (auto &AdditionalFile : Repo.m_AdditionalConfigFiles)
			{
				if (AdditionalFile.f_IsEmpty())
					continue;
				fAssignConfigFile(AdditionalFile, AdditionalFilesByRepo);
			}
		}

		// Detect Perforce root and collect Perforce-managed config files.
		// Mirrors live in PerforceRootMirrorFiles, kept separate from the
		// canonical PerforceRootConfigFiles for the same reason as the git
		// path: they must be staged with the changelist but must not feed
		// RootSeed parsing or fg_CollectChildCommits, where their duplicated
		// hash entries would corrupt start-commit resolution.
		...
		if (bIsPerforceRoot)
		{
			...
			for (auto &pRepository : *pRepositoryByLocation)
			{
				for (auto &AdditionalFile : pRepository->m_AdditionalConfigFiles)
				{
					if (AdditionalFile.f_IsEmpty())
						continue;
					CStr ConfigDir = CFile::fs_GetPath(AdditionalFile);
					...
					if (!bInsideGitRepo && !PerforceRootConfigFiles.f_FindEqual(AdditionalFile))
						PerforceRootMirrorFiles[AdditionalFile];
				}
			}
		}
		...
		for (auto &Repos : FilteredRepos.f_Reverse())
		{
			for (auto *pRepo : Repos)
			{
				...
				auto *pAdditionalFiles = AdditionalFilesByRepo.f_FindEqual(Repo.m_Location);
				bool bHasConfig = pConfigFiles && !pConfigFiles->f_IsEmpty();
				bool bHasAdditional = pAdditionalFiles && !pAdditionalFiles->f_IsEmpty();
				if (!bHasConfig && !bHasAdditional)
					continue;
				...
				// Union of canonical + mirror files for staging operations:
				// existence checks, AllConfigRelPaths, status preflight. Mirrors
				// must reach `git add` so the committed copy stays in lockstep
				// with the canonical, but they never feed semantic operations
				// (start-commit resolution, child-commit collection) — those use
				// `*pConfigFiles` directly.
				TCSet<CStr> StageFiles;
				if (pConfigFiles)
					StageFiles += *pConfigFiles;
				if (pAdditionalFiles)
					StageFiles += *pAdditionalFiles;

				// A managed config file that is missing from the workspace is
				// always an error: repo-commit will not delete config files
				// automatically, the user must restore the file or remove the
				// corresponding entry from the build configuration.
				for (auto &ConfigFile : StageFiles)
				{
					if (!CFile::fs_FileExists(ConfigFile))
					{
				...
				// Preflight gets the full StageFiles union so mirrors are
				// status-checked alongside canonicals; child-commit collection
		...
		if (bIsPerforceRoot && (!PerforceRootConfigFiles.f_IsEmpty() || !PerforceRootMirrorFiles.f_IsEmpty()))
		{
			...
			for (auto &ConfigFile : PerforceRootConfigFiles)
				ConfigFilePaths.f_Insert(ConfigFile);
			for (auto &MirrorFile : PerforceRootMirrorFiles)
				ConfigFilePaths.f_Insert(MirrorFile);
