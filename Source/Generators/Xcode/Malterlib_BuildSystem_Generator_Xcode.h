...
namespace NMib::NBuildSystem::NXcode
{
	...
	private:
		...
		struct CElement
		{
			...
			void f_SetValue(CStr const &_Value)
			{
				m_bUseValues = false;
				m_Value = _Value;
			}
			bool f_IsSameValue(CElement const &_Other) const
			{
		...
		void fp_SetEvaluatedValues(
			...
			, TCMap<CConfiguration, CConfigResult>& _Result) const;
